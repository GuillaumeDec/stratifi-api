...
class Normalizer:
   def __init__(self):
      self.DICT_STOCKS = self.DoTraining(self.GiveWords(open('/Users/Dess/Workspace/stratifi-api/data/new_norm_stocks.txt','r').read()))
      self.alphabet = 'abcdefghijklmnopqrstuvwxyz'

   def GiveWords(self,text):
      # we only keep actual words, without fancy characters except amp, whitespace and dash
      return re.findall('[a-z& \-]+', text.lower()) 
   
   def DoTraining(self,features):
      """
   ...
   def known_edits2(self,word):
      ...
      """
      return set(e2 for e1 in self.edits1(word) for e2 in self.edits1(e1) if e2 in self.DICT_STOCKS)
   
   def known(self, words): return set(w for w in words if w in self.DICT_STOCKS)
   ...
   def normalize(self, word):
      ...
      return max(candidates, key=self.DICT_STOCKS.get)
   ...
   def filter(self,stock):
      ...
      self.filters = [r'\binc.?\b',r'\bltd.?\b',r'\bhldg.?\b',r'\bholdings?.?\b',r'\bin.?\b',r'\bcompany\b',r'\benterprises?\b',r'\bag.?\b',
...
def handle_compounds(in_stock):
   ...
   """
   in_stock = theNormalizer.normalize(theNormalizer.filter(in_stock)).split()
   score = 0
...
def GetFunds(in_stock, df):
   ...
   ##########
   the_stock = theNormalizer.normalize( theNormalizer.filter(in_stock) )
   if the_stock in normalized_stocks:
      matched_funds = df[df['filtered_positions'].apply( lambda x: any(the_stock in s for s in x) )]['company_name'].tolist()
      outdict = {}
      for m in matched_funds:
